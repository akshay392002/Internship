...
create table user_role( role_id int primary key, role_name varchar(50) unique);
create table users(user_id int primary key , role_id int, foreign key(role_id) references user_role(role_id), first_name varchar(50), 
last_name varchar(50), contact_number varchar(10) check(length(contact_number) = 10), email varchar(100));
create table patient(patient_id int primary key , first_name varchar(50), last_name varchar(50), birth_date date, gender enum(
'Male', 'Female', 'Other'), contact_number varchar(10) check(length(contact_number) = 10), email varchar(100),insurance_id int,
 foreign key(insurance_id) references insurance(insurance_id));
 create table diagnosis(diagnosis_id int primary key ,patient_id int,doctor_id int,foreign key(patient_id) references patient(patient_id),
 foreign key(doctor_id) references users(user_id), reason varchar(200), diagnosis_date date, diagnosis_amount int);
 create table insurance(insurance_id int primary key, insurance_company varchar(100), expiry_date date);
 create table bill (bill_id int primary key  auto_increment, patient_id int,foreign key(patient_id) references patient(patient_id), bill_date date,
 bill_amount int);
 ...
 select p.*,d.doctor_id,d.reason,d.diagnosis_date, i.insurance_company, i.expiry_date,b.bill_date,b.bill_amount from patient p join diagnosis d on p.patient_id = 
 d.patient_id join insurance i on i.insurance_id = p.insurance_id join bill b on b.patient_id = p.patient_id where p.patient_id = 100;
